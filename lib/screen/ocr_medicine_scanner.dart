...
class _OcrMedicineScannerState extends State<OcrMedicineScanner> {
  ...
  List<String> medicineNames = const [
    "Paracetamol", "Ibuprofen", "Amoxicillin", "Cetirizine", "Loperamide",
    "Mefenamic Acid", "Losartan", "Metformin", "Salbutamol", "Omeprazole",
    "Aspirin", "Cotrimoxazole", "Dextromethorphan", "Diphenhydramine",
    "Domperidone", "Hydroxyzine", "Ciprofloxacin", "Doxycycline", "Carbocisteine",
    "Ambroxol", "Clarithromycin", "Ranitidine", "Simvastatin", "Amlodipine",
    "Metoprolol", "Hydrochlorothiazide", "Candesartan", "Glibenclamide",
    "GLIBENCLAMIDE", "Levothyroxine", "Prednisone", "Nighttime Cough DM",
    "PRETENDMED", "Pretenmed", "Fakelixir",
  ];

  List<String> brandNames = const [
    "Biogesic", "Medicol", "Flanax", "Neozep", "Tuseran", "Dolfenal", "Solmux",
    "Alaxan", "Ritemed", "Pharex", "Rovamycine", "Zinnat", "Augmentin", "Ponstan",
    "Robitussin", "Sarbena",
  ];

  Map<String, String> keywordMapping = const {
    "Exp:": "Expiration Date", "Exp Date": "Expiration Date", "Exp.": "Expiration Date",
    "Expiring": "Expiration Date", "Expires": "Expiration Date", "Valid Until": "Expiration Date",
    "Expiry": "Expiration Date", "Use by": "Expiration Date"
  };
  ...
  Future<void> _processImage() async {
    ...
    try {
      ...
      for (TextBlock block in recognizedText.blocks) {
        for (TextLine line in block.lines) {
          ...
          for (String medicine in medicineNames) {
            if (lineText.contains(medicine.toLowerCase()) && detectedData["Medicine Name"]!.isEmpty) {
          ...
          for (String brand in brandNames) {
            if (lineText.contains(brand.toLowerCase()) && detectedData["Brand Name"]!.isEmpty) {
              detectedData["Brand Name"] = brand;
              break;
            }
          }

          RegExp dosageRegex = RegExp(r'(\d+(\.\d{1,2})?\s*(mg|mg\.|milligrams?|mgs?))');
          ...
          for (String keyword in keywordMapping.keys) {
            if (lineText.contains(keyword.toLowerCase()) && detectedData["Expiration Date"]!.isEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Error processing the image. Please try again.")),
      );
    }
  }
  ...
  void _showConfirmationDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text("Are you sure you want to add this medicine to the list?"),
        actions: [
          TextButton(
            onPressed: () {
  ...
  void _acceptData() {
    Map<String, String> finalData = extractedDataControllers.map((key, controller) => MapEntry(key, controller.text));
    setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    final Color primaryColor = Colors.blue.shade500; // Example primary color for BARMED
                      ...
                      Row(
                        ...
                        children: [
                          ElevatedButton.icon(
                            ...
                            icon: Icon(Icons.photo_library, color: isDarkMode ? Colors.white : Colors.black87),
                            label: Text("Gallery", style: TextStyle(color: isDarkMode ? Colors.white : Colors.black87)),
                          ...
                          ElevatedButton.icon(
                            ...
                            icon: Icon(Icons.camera_alt, color: isDarkMode ? Colors.white : Colors.black87),
                            label: Text("Camera", style: TextStyle(color: isDarkMode ? Colors.white : Colors.black87)),
                            style: ElevatedButton.styleFrom(
                              foregroundColor: isDarkMode ? Colors.white : Colors.black87,
                              backgroundColor: isDarkMode ? Colors.grey[800] : primaryColor,
                              padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12),
  ...
  if (_hasImage)
              Expanded(
                child: SingleChildScrollView(
                  child: Column(
                    children: extractedDataControllers.entries
                        .map((entry) {
                      return Card(
                        elevation: 4,
                        ...
                        child: ListTile(
                          title: Text(entry.key, style: TextStyle(fontWeight: FontWeight.bold, color: isDarkMode ? Colors.white : Colors.black)),
                          subtitle: TextField(
                            ...
                            decoration: InputDecoration(
                              ...
                              hintText: _isEditingEnabled ? "Edit here" : null,
                            ...
                            keyboardType: entry.key == "Stocks" ? TextInputType.number : null,
                          ),
                        ),
                      );
                    }).toList(),
